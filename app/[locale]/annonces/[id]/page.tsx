...
export default async function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const formatPrice = (price: number) => {
    if (price === 0) return isSadaqa ? t("price.freeDonation") : t("price.notSpecified")
  ...
  const conditionLabels: Record<string, string> = {
    neuf: t("conditions.new"),
    comme_neuf: t("conditions.likeNew"),
    bon_etat: t("conditions.good"),
    usage_normal: t("conditions.normal"),
    pour_pieces: t("conditions.parts"),
  }

  const whatsappNumber = listing.whatsappNumber || "+33612345678"
  const whatsappLink = `https://wa.me/${whatsappNumber.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(t("whatsappMessage", { title: listing.title }))}`
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-6">
              <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>{t("views", { count: listing.viewsCount })}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  {isSadaqa && <Badge className="bg-sadaqa text-sadaqa-foreground">{t("sadaqaBadge")}</Badge>}
                </div>
              </div>
            ...
            {isBtp && (listing.skills || listing.experience) && (
              <Card className="mb-6 border-orange-200 bg-orange-50/30">
                <CardContent className="p-6">
                  ...
                  {listing.skills && (
                    <div className="mb-3">
                      <p className="mb-2 text-sm font-medium text-muted-foreground">{t("btp.skills")}</p>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {listing.experience && (
                    <div>
                      <p className="mb-1 text-sm font-medium text-muted-foreground">{t("btp.experience")}</p>
            ...
            <Card className="mb-6">
              <CardContent className="p-6">
                <h2 className="mb-4 text-xl font-semibold text-foreground">{t("description")}</h2>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-20 space-y-4">
              ...
              <Card>
                <CardContent className="p-6">
                  <h2 className="mb-4 text-lg font-semibold text-foreground">{t("seller.title")}</h2>
                  <Link href={`/profil/${listing.userId}`} className="mb-4 flex items-center gap-3 hover:opacity-80">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">{t("seller.member")}</p>
                    </div>
                  </Link>
                  ...
                  {isOwner ? (
                    <div className="space-y-2">
                      <Button asChild className="w-full bg-transparent" variant="outline">
                        <Link href={`/annonces/${listing.id}/modifier`}>{t("seller.editListing")}</Link>
                      </Button>
                      <p className="text-center text-sm text-muted-foreground">{t("seller.yourListing")}</p>
                    </div>
                  ) : (
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="mb-3 text-sm font-semibold text-foreground">{t("safetyTips.title")}</h3>
                  <ul className="space-y-2 text-xs text-muted-foreground">
                    ...
                    <li className="flex gap-2">
                      <span className="text-primary">•</span>
                      <span>{t("safetyTips.verifyBeforePay")}</span>
                    </li>
                    <li className="flex gap-2">
                      <span className="text-primary">•</span>
                      <span>{t("safetyTips.neverPayAdvance")}</span>
                    </li>
                    <li className="flex gap-2">
                      <span className="text-primary">•</span>
